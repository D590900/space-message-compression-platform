...
The API additionally reconciles PostgreSQL to Valkey every `JOB_OUTBOX_RECONCILE_MS` (60 seconds by default). A `PENDING` or `FAILED_RETRYABLE` compression, decompression or capsule record is re-enqueued only when it has no unpublished outbox event and its last successful publication is older than `JOB_OUTBOX_STALE_MS` (30 minutes by default). A PostgreSQL advisory lock serializes this scan across API replicas. Keep the stale interval at least as long as the worker claim-idle threshold; active intermediate states are deliberately excluded so reconciliation cannot restart work that is still running.
