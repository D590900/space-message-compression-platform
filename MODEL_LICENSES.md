...
| SNAC                | `8f79a718f1ad71f94f79999f0071348227aff22e` | MIT                   | MIT — 24 kHz checkpoint at HF revision `d73ad176…`         | Runtime pending |
...
Pinned evidence URLs and exact disable reasons are in `services/compression-worker/model-manifests/catalog.json`. The validator rejects unknown weight terms, missing hashes, mutable decoder references or incomplete input contracts for any entry marked enabled. CoD-Lite is the first enabled neural pipeline; its CUDA runtime is pinned to the immutable digest recorded in the catalog and weights remain external. SNAC now has separately verified MIT checkpoint terms, immutable hashes, a real adapter and a reproducible CPU runtime definition, but stays fail-closed until that runtime is published and its digest recorded. Every other unregistered family still fails closed if toggled on without implementation work.
