...
| CoD-Lite / GenCodec | `c49eb0d643cc75e6c732cbc311a508627b54cf06` | MIT                   | MIT — 0.0312-bpp checkpoint at HF revision `cfda8135…`     | Runtime disabled |
| SNAC                | `8f79a718f1ad71f94f79999f0071348227aff22e` | MIT                   | UNKNOWN — checkpoint-specific verification required        | Disabled         |
...
Pinned evidence URLs and exact disable reasons are in `services/compression-worker/model-manifests/catalog.json`. The validator rejects unknown weight terms, missing hashes, mutable decoder references or incomplete input contracts for any entry marked enabled. CoD-Lite is the first checkpoint with approved weight terms and a registered adapter, but it remains unavailable until its pinned CUDA runtime/decoder image is packaged and verified. This release has no enabled neural pipeline, so unregistered families still fail closed if toggled on without implementation work.
