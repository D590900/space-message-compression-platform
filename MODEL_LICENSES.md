...
| EnCodec / Transformers | `c1c34249fa27deefbd4a377dfbf883a39baf5c6d` | Apache-2.0            | MIT — official 48 kHz model at HF revision `c3def8e7…`     | Runtime pending |
...
Pinned evidence URLs and exact disable reasons are in `services/compression-worker/model-manifests/catalog.json`. The validator rejects unknown weight terms, missing hashes, mutable decoder references or incomplete input contracts for any entry marked enabled. CoD-Lite, SNAC and Mimi have immutable decoder digests recorded in the catalog and keep weights external. EnCodec has approved MIT checkpoint terms, exact hashes and a real adapter, but remains fail-closed until its scanned decoder runtime is published and pinned. The other four unregistered families still fail closed if toggled on without implementation work.
